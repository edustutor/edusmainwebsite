      ...
      <div className="container-edge">
        ...
        >
          <div aria-hidden className="absolute inset-0 -z-10">
            <AmbientGlow top="-8%" left="-4%"  size={200} color="#2563EB" opacity={[0.14, 0.22]} duration={20} blur={80} />
            <AmbientGlow top="-8%" right="-4%" size={200} color="#06B6D4" opacity={[0.12, 0.20]} duration={22} delay={2} blur={80} />
            <AmbientGlow bottom="-12%" left="35%" size={220} color="#8B5CF6" opacity={[0.10, 0.18]} duration={24} delay={4} blur={80} />
            <AmbientGlow bottom="-12%" right="20%" size={180} color="#FACC15" opacity={[0.10, 0.18]} duration={18} delay={1} blur={70} />
          </div>

          <m.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={inViewClose}>
            <m.p variants={fadeUp} className="eyebrow"><span className="dot" />Start Your Journey</m.p>
            <m.h2
              variants={fadeUp}
              className="heading mt-5"
              style={{ fontSize: "clamp(28px, 3.4vw, 44px)", lineHeight: 1.15 }}
            >
              Start your child&apos;s global learning journey with <em>EDUS.</em>
            </m.h2>
            <m.p variants={fadeUp} className="text-[#2B3950] text-[15.5px] max-w-2xl mx-auto mt-5 leading-[1.7]">
              Expert tutors, personalized one to one teaching, flexible online learning, and
              caring academic guidance from anywhere in the world.
            </m.p>
